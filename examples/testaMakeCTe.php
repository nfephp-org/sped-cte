...
$cteTools->aConfig['cnpj']="81450900000132";         // CNPJ do emitente
$cteTools->aConfig['ie']="4220816074";             // Inscricao estadual
$cteTools->aConfig['razaosocial']="BARTHOLO TRANSPORTES RODOVIARIOS LTDA"; // Razao social
$cteTools->aConfig['nomefantasia']="BARTHOLO TRANSPORTES RODOVIARIOS LTDA"; // Nome fantasia
$siglaUF="PR";
$cMunEnv01='4108304'; //codigo do municipio de envio
$xMunEnv01='FOZ DO IGUACU';
$numeroCTE = "60";

$chave = $cte->montaChave(    
    $cUF = $cteTools->getcUF($siglaUF), // Retorna o codigo da UF
...
$resp = $cte->ideTag(
    $cUF = $cteTools->getcUF($siglaUF),                // Codigo da UF da tabela do IBGE
    ...
    $CFOP = '5351',             // Codigo fiscal de operacoes e prestacoes
    ...
    $cMunIni = $cMunEnv01,       // Utilizar a tabela do IBGE. Informar 9999999 para as operações com o exterior.
    $xMunIni = $xMunEnv01, // Informar 'EXTERIOR' para operações com o exterior.
    $UFIni = $siglaUF,              // Informar 'EX' para operações com o exterior.
...
$resp = $cte->enderEmitTag(
    ...
    $cMun = $cMunEnv01,                  // Código do município (utilizar a tabela do IBGE)
    $xMun = $xMunEnv01,                  // Nome do municipio
    $CEP = '85863150',                   // CEP
    $UF = $siglaUF,                      // Sigla UF
...
$resp = $cte->remTag(
    ...
    $xNome = 'CT-E EMITIDO EM AMBIENTE DE HOMOLOGACAO - SEM VALOR FISCAL',  // nome do remetente
...
$fileAssinado = "../xml/homologacao/assinadas/{$chave}-cte.xml";

header('Content-type: text/html; charset=UTF-8');
if ($resp) {    
    $xml = $cte->getXML();
    file_put_contents($fileAssinado, $xml);    
} else {    
...
$xml = file_get_contents($fileAssinado);
$xml = $cteTools->assina($xml);
file_put_contents($fileAssinado, $xml);

if (! $cteTools->validarXml($fileAssinado) || sizeof($cteTools->erros)) {
...
print_r("<pre>validação do xml...ok</pre>");
...
if( $aRetorno['cStat'] == '103' ){ // Lote entregue na SEFAZ com sucesso
    
    // Consultar com o recibo para verificar se o lote foi autorizado para uso
    $recibo = $aRetorno['nRec']; // numero do recibo    
    $retorno = $cteTools->sefazConsultaRecibo($recibo, $tpAmb, $aRetorno);

    if($aRetorno['cStat'] == '104'){ // Lote encontrado na SEFAZ 
        
        if($aRetorno['aProt']['cStat'] =='100'){ // Autorizado o uso do CT-e
            $xml = file_get_contents($fileAssinado);
            $fileAprovado = "../xml/homologacao/enviadas/aprovadas/{$chave}-cte.xml";
            file_put_contents($fileAprovado, $xml); // adiciona na pasta aprovadas
            unlink($fileAssinado); // remove da pasta assinadas
            echo 'Autorizado o uso do CT-e:<pre>';            
            print_r($aRetorno);
            echo "</pre>";
        }else{
            unlink($fileAssinado); // remove da pasta assinadas ( nao aprovou )
            print_r("<pre>resposta da SEFAZ:</pre><pre>");
            print_r($aRetorno);
            echo "</pre>";
        }        
